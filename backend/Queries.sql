...
SELECT CONCAT(e.fname," ",e.lname) AS Display_name, p.Player_ID, r.Name AS Ranking, p.xp_Total AS "Total XP", p.Level, ((req.value * r.level) * p.level) AS Next_Level, ((req.value * r.level) * p.level) - p.xp_total AS xp_Needed
